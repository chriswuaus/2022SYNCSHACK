...
const DegreeEntry = ({ onAdd }: DegreeEntryProps) => {
    ...
    return <div className={styles.container}>
        ...
        <TextField sx={{ m: 1, maxWidth: 320 }} type="number" value={year} label="Start Year" className={styles.padded} onChange={ev => { setYear(ev.target.value) }} />
        <FormControl sx={{ m: 1, maxWidth: 320 }}>
            <InputLabel id="semester-label">Semester</InputLabel>
            <Select id="Semester" value={semester} onChange={ev => { setSemester(ev.target.value) }} className={styles.padded} >
        ...
        <center>
            <Button
                ...
                variant="contained"
                onClick={() => onAdd({ degreeName: degree!, major: major!, degreeYear: year!, semester: semester!, link: `/degrees/{degree+year+major}` })}
                disabled={!(degree != undefined && major != undefined && year != undefined && semester != undefined)}>
                Add
            </Button>
        </center>
        <p>
